...
{
    public sealed class EnemyAITurnDriver : MonoBehaviour // Battle 씬의 EnemyTurn을 일반 AI와 2x2 보스 기본 전투에 자동 연결하는 런타임 드라이버
    {
        ...
        private readonly EnemyAIAdvancedPlanner _planner = new EnemyAIAdvancedPlanner(); // 35일차 일반 적 Base·Role·Threat·Special 최종 플래너
        private readonly BossActionPlanner _bossPlanner = new BossActionPlanner(); // 38일차 2x2 보스 이동·공격 기본 플래너
        private BossPhase2Controller _bossPhase2Controller; // 39일차 Phase 2 전환·텔레그래프·범위 공격을 EnemyTurn보다 먼저 처리할 관리자
        ...
        private static void AutoCreateForBattleScene() // 씬에 수동 컴포넌트 배치 없이 AI 드라이버를 만드는 부트스트랩
        {
            ...
            var driverObject = new GameObject("EnemyAITurnDriver_Day39"); // 일반 AI+보스 통합 턴 드라이버 전용 오브젝트 생성
        ...
        private IEnumerator Start() // BattleController 자동 생성 순서와 무관하게 안전하게 참조를 찾는 초기화 코루틴
        {
            ...
            Debug.LogError("38일차 Enemy AI 초기화 실패: BattleController, BoardInputController 또는 BoardView를 찾지 못했습니다."); // 제한 시간 내 연결 실패 안내
        }
        ...
        private void Bind() // 기존 전투 흐름에 AI를 한 번만 연결하는 메서드
        {
            ...
            EnsureBossPhase2Controller(); // 39일차 Phase 2 관리자도 같은 전투 상태에 연결
            ...
            Debug.Log("39일차 Enemy AI 연결 완료: Phase 2 텔레그래프를 우선 처리하고, 그 외에는 일반 AI와 Phase 1 보스 행동을 비교합니다."); // 개발 로그 출력
        }
        ...
        private IEnumerator ExecuteEnemyTurnNextFrame(int turnNumber) // 현재 EnemyTurn에서 일반 적과 보스 후보를 비교해 행동 하나를 선택·실행하는 코루틴
        {
            ...
            bool hasNormalAction = _planner.TryChooseAction(board, out var normalAction); // 35일차 일반 적 최고 후보 계산
            bool hasBossAction = _bossPlanner.TryChooseAction(board, out var bossAction); // 38일차 보스 최고 후보 계산
            ...
            else // 일반 적과 보스 모두 합법 행동이 하나도 없으면
            {
