...
namespace ProjectEta.Run // 런(플레이 세션) 관련 타입을 모아두는 네임스페이스
{
    public class RunState // 런 전체 상태(보드·덱·손패·킹 체력 등)를 담는 최상위 클래스
    {
        ...
        public FusionDiscoveryLog FusionDiscovery { get; } // 이 런에서 발견한 숨김 합성식 기록
        ...
        public RunState(int startingKingHp) // 런 상태 생성자
        {
            ...
            FusionDiscovery = new FusionDiscoveryLog(); // 숨김 합성식 발견 기록을 빈 상태로 생성
        ...
        public RunSaveData ToSaveData() // 현재 런 상태를 저장용 데이터로 변환하는 메서드
        {
            ...
            foreach (var recipeId in FusionDiscovery.DiscoveredRecipeIds) // 이번 런에서 발견한 숨김 합성식을 순회하며
            {
            ...
            foreach (var card in Deck.OwnedCardPool) // 현재 보유 풀 위치에 있는 카드를 순회하며
            {
                data.ownedCardPoolIds.Add(card.PieceId); // PieceId를 저장 목록에 추가
            }

            foreach (var card in Deck.DrawPile) // 현재 드로우 순서를 순회하며
            {
        ...
        public static RunState FromSaveData(RunSaveData data, PieceDatabase database) // 저장용 데이터로부터 런 상태를 복원하는 정적 메서드
        {
            ...
            runState.FusionDiscovery.Restore(data.discoveredRecipeIds); // 저장된 숨김 합성식 발견 기록을 복원
            ...
            if (data.drawPileIds != null) // 드로우 목록이 없는 구버전 저장도 안전하게 처리
            {
            ...
            foreach (var pieceId in data.deadCardPileIds) // 저장된 죽은 카드 더미 id 목록을 순회하며
            {
                ...
                if (definition != null) // 정의를 찾았으면
                {
                    runState.Deck.MoveToDeadPile(definition); // 죽은 카드 더미 위치를 복원
                }
            }
        ...
        public int CountOwnedCopies(PieceDefinition definition) // 4·5성 수량 제한에 쓰는 동일 기물의 영구 보유 수를 세는 메서드
        {
            if (definition == null) return 0; // 기준 기물이 없으면 0개

            int count = 0; // 누적 보유 수

            foreach (var card in Deck.OwnedCardPool) // 정상 보유 풀 위치의 카드를 순회하며
            {
                if (card == definition) count++; // 같은 기물이면 누적
            }

            foreach (var card in Deck.DeadCardPile) // 현재 라운드에서 사망해 죽은 카드 더미로 이동한 카드도 순회하며
            {
                if (card == definition) count++; // 사망은 영구 소유권 상실이 아니므로 보유 수에 포함
            }

            return count; // 보유 풀과 죽은 카드 더미를 합친 영구 보유 수 반환
        }

        public int CountDeployedCopies(PieceDefinition definition) // 보드 위 아군 기물 중 동일 기물이 몇 개 배치돼 있는지 세는 메서드
        {
            if (definition == null) return 0; // 기준 기물이 없으면 0개
